...
x = np.array(
    [[7, 53, 183, 439, 863, 497, 383, 563,
     ...
     [813, 883, 451, 509, 615, 77, 281, 613,
      459, 205, 380, 274, 302, 35, 805]]
)
"""
x = np.array([[7, 53, 183, 439, 863],
              [497, 383, 563, 79, 973],
              [287, 63, 343, 169, 583],
              [627, 343, 773, 959, 943],
              [767, 473, 103, 699, 303]])
...
def improve(x, i, j, best_score):
    for cycle_length in range(2, x.shape[0]+1):
        for idxs in combinations(i, cycle_length):
            combs = [idxs[k:]+idxs[:k] for k in range(1, len(idxs))]
            for comb in combs:
                i_copy = i.copy()
                i_copy[idxs,] = i_copy[comb,]
                score = x[i_copy, j].sum()
                if score > best_score:
                    return score, i_copy
    return None, None
